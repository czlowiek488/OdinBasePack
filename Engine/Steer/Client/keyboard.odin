...
loadKeyboardMapping :: proc(
	module: ^Module($TEventLoopTask, $TEventLoopResult, $TError),
...
handleKeyboardSDLEvent :: proc(
	...
	if !found {
		if module.keyboard do log.debugf("Button Pressed - {} > {}", event, keyId)
		return
	}
	if module.keyboard do log.debugf("Button Pressed - {} > {} > {}", event, keyId, keyboardEvent.name)
	switch event {
	case .INVALID:
