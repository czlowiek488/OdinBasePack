...
@(require_results)
vectorFromAngle :: proc(angle: f32) -> (vector: Vector) {
...
@(require_results)
scaleToFit :: proc(itemSize: Vector, targetSize: Vector) -> (vector: Vector) {
...
@(require_results)
toRadians :: proc(angle: f32) -> (radians: f32) {
	radians = angle * math.PI / 180
	return
}

@(require_results)
rotateVector :: proc(vector: Vector, center: Vector, angle: f32) -> (rotated: Vector) {
	radians := toRadians(angle)
...
@(require_results)
rotateTriangle :: proc(triangle: Triangle, angle: f32) -> (rotated: Triangle) {
